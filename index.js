...
import { AppRegistry, Platform, Alert, AlertIOS } from 'react-native';
import React, { Component } from 'react';
import EulaPage from './views/eula';
import HomePage from './views/home';
import SearchPage from './views/search';
import SearchResultsPage from './views/search-results';
import PriceUpdatesPage from './views/price-updates';
import AddPriceUpdatePage from './views/add-price-update';
...
import { Button, StyleSheet, Text, View, TouchableHighlight, TextInput, ListView } from 'react-native';
import { Icon } from 'react-native-elements';
import { createAppContainer, StackNavigator } from 'react-navigation';
import { createDrawerNavigator } from 'react-navigation-drawer';
import { createStackNavigator } from 'react-navigation-stack';
import CustomTableCell from './components/custom-table-cell';
import CustomTableCellNotificationsList from './components/custom-table-cell-notifications';
...
const MainNavigation = createAppContainer(createStackNavigator({
    ...
    Search: {
        ...
        navigationOptions: ({navigation}) => ({
            ...
            title: typeof(navigation.state.params) === 'undefined' || typeof(navigation.state.params.title) === 'undefined' ? 'Search' : navigation.state.params.title,
            headerRight: ''
        })
    },
    SearchResults: {
        ...
        navigationOptions: ({navigation}) => ({
            ...
            title: typeof(navigation.state.params) === 'undefined' || typeof(navigation.state.params.title) === 'undefined' ? 'Search Results' : navigation.state.params.title,
            headerRight: ''
        })
    },
    PriceUpdates: {
        screen: PriceUpdatesPage,
        navigationOptions: ({navigation}) => ({
            title: 'Price Updates',
            headerRight: <Text onPress={() => {
            // send selected party id to next view
            var notificationId = navigation.getParam('id', null);
            navigation.navigate('AddPriceUpdate', {id: notificationId});
            }} style={styles.headerBtn}>+</Text>,
            headerTintColor: '#fff',
            headerStyle: {
                backgroundColor: '#ec2227'
            }
    ...
    AddPriceUpdate: {
        screen: AddPriceUpdatePage,
        navigationOptions: ({navigation}) => ({
            headerTintColor: '#fff',
            headerStyle: {
                backgroundColor: '#ec2227'
            },
            title: typeof(navigation.state.params) === 'undefined' || typeof(navigation.state.params.title) === 'undefined' ? 'Add Price Update' : navigation.state.params.title,
            headerRight: ''
        })
    },
    Eula: {
...
AppRegistry.registerComponent('CustomTableChecklist', () => CustomTableChecklist);
AppRegistry.registerComponent('CustomTableCellNotificationsList', () => CustomTableCellNotificationsList);
